    ...
    public class UsuarioService
    {
        ...
        public async Task InsertarAsync(Usuario u)
        {
            ...
            string query = @"
                ...
                VALUES ('USR' + RIGHT('00' + CAST((SELECT ISNULL(MAX(id),0) + 1 FROM usuarios) AS VARCHAR), 2), 
                        @Nombres, @Password, @Rol, @Estado, GETDATE(), GETDATE())";

            using (var cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@Nombres", u.Nombres);
                cmd.Parameters.AddWithValue("@Password", u.Password);
                cmd.Parameters.AddWithValue("@Rol", u.RolUsuarioId);
        ...
        // =========================================================================
        // MOTOR DE ROLES (Para cargar ComboBoxes de forma dinámica y limpia)
        // =========================================================================
        ...
        // =========================================================================
        // MOTOR DE MATRIZ DE SEGURIDAD (PERMISOS GRANULARES)
        // =========================================================================
        ...
        public async Task<List<RolPermiso>> ObtenerPermisosPorRolAsync(int rolUsuarioId)
        {
            ...
            // Esta consulta es potente: trae todos los módulos del sistema y, si el rol tiene configuración, la cruza; si no, devuelve falso por defecto de forma segura.
            string query = @"
                SELECT 
                    m.id AS modulo_id,
                    m.codigo_modulo,
            ...
            using (var cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@RolId", rolUsuarioId);
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
                    {
        ...
        public async Task GuardarPermisosMasivosAsync(int rolUsuarioId, List<RolPermiso> matrizPermisos)
        {
            ...
            // Usamos una transacción para asegurar que se guarden todos los permisos o ninguno, evitando datos corruptos
            ...
            try
            {
                ...
                {
                    string query = @"
                        IF EXISTS (SELECT 1 FROM rol_permisos WHERE rol_usuario_id = @RolId AND modulo_id = @ModId)
                        BEGIN
                    ...
                    {
                        cmd.Parameters.AddWithValue("@RolId", rolUsuarioId);
                        cmd.Parameters.AddWithValue("@ModId", permiso.ModuloId);
                        cmd.Parameters.AddWithValue("@Ver", permiso.PuedeVer);
                        cmd.Parameters.AddWithValue("@Crear", permiso.PuedeCrear);
                        cmd.Parameters.AddWithValue("@Editar", permiso.PuedeEditar);
                        cmd.Parameters.AddWithValue("@Eliminar", permiso.PuedeEliminar);
                        cmd.Parameters.AddWithValue("@Imprimir", permiso.PuedeImprimir);

                        await cmd.ExecuteNonQueryAsync();
                    }
                }
                trans.Commit(); // Consistencia total garantizada
            }
            catch
            {
                trans.Rollback();
